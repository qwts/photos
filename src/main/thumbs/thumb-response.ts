...
export async function handleThumbRequest(service: ThumbService, admit: () => void, request: Request): Promise<Response> {
  try {
    admit();
  } catch {
  ...
  const loaded = await service.getThumb(parsed.photoId, parsed.size, request.signal);
