...
export function useNativeCommandRouter(deps: NativeCommandRouterDeps): void {
  const {
    nativeCommand,
    state,
    dispatch,
    ...
    switch (nativeCommand.id) {
      ...
      case 'photo.restore':
        ...
        void window.overlook.library.restore({ photoIds: targetIds }).then(({ restored }) => {
          dispatch({ type: 'toast/shown', toast: { title: `Restored ${restored} ${restored === 1 ? 'photo' : 'photos'}`, tone: 'green' } });
        });
        return;
