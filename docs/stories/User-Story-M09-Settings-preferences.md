...
Lane C — starts early (the settings store needs only M01's IPC). Typed, persisted app settings surfaced in the 640px three-pane SettingsDialog: **General** (sort order and live Dark/Light/System appearance), **Storage & Backup** (provider connection card + quota, auto-backup, copy/move, Wi-Fi-only, bandwidth slider; backup controls disabled when disconnected), **Privacy** (E2E encryption badge always-on, share-diagnostics off by default; face-grouping row ships disabled — the feature is deferred by design despite the mock showing it locked-on; conflict recorded).
...
| General: sort order (Date/Name/Size) re-orders the grid LIVE and persists — keyset-safe `order` param with matching indexes (migration v3, query-plan pinned); appearance applies Dark/Light/System live and across relaunch; thumbnails locked on | ✅ #113, #395     | e2e + stories + theme runtime tests — ledger id `m09-settings-general`                                                                                                                 |
