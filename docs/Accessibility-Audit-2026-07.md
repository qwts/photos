...
| 17  | **Visible label ≠ accessible name (2.5.3)**, systemically — via `Field.tsx` (used by every settings pane), `KeyDialog`'s label divs, `ExportDialog`'s row spans, and `ProtectedAlbumCeremony`, where a wrapping `<label>` is _overridden_ by the field's own `aria-label`. | [#400](https://github.com/qwts/photos/issues/400)                                                     |
