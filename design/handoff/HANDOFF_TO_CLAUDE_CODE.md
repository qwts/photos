...
- **Color:** cool near-black neutrals (`--gray-0` window → `--gray-2` cards); four semantic accents sharing L/C in oklch — cyan (selection/primary), amber (cloud/offloaded), green (encrypted/verified), red (destructive); each has a `-dim` 16%-alpha fill.
