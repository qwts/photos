...
Locator: getByRole('button', { name: 'Cancel and keep staged progress' })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('button', { name: 'Cancel and keep staged progress' })
...
- main:
  - region "Cloud library recovery":
    ...
    - strong: Ready to restore this library.
    - list:
      - listitem: Disk space is checked before any originals download.
      - listitem: Downloads are staged, verified, and resumable after cancellation.
      - listitem: The current library remains active unless the complete staged restore validates.
      - listitem: Activation uses rollback-safe replacement, then Overlook relaunches.
    - button "Back"
    - button "Restore 4 photos"
    - alert:
