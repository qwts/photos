...
- Releases are cut by merging the bot-maintained **Version packages** PR (the
  ...
  merge is tagged `v0.x.y` automatically and the Release workflow publishes an
  unsigned pre-release (mac + win builds, CHANGELOG notes, design-package zip).
  Never hand-tag versions or hand-run `changeset version`.
