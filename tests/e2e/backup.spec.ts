...
test('forced upload error: red retry toast + error glyph', async () => {
  ...
  try {
    ...
    await page.getByRole('button', { name: 'Back up' }).click();
    await expect(page.getByTestId('screen-reader-announcer-assertive')).toContainText('Backup: 1 failed — will retry', {
      timeout: 30_000,
    });
...
test('integrity repair and remote-only loss update in place without clearing selection', async () => {
  ...
  try {
    ...
    await expect(page.getByTestId('screen-reader-announcer-polite')).toContainText('Backup repaired: 1 cloud copies');
    ...
    await page.evaluate(`window.overlook.backup.run({})`);
    await expect(page.getByTestId('screen-reader-announcer-assertive')).toContainText('Backup damaged: 1 originals missing');
    await expect(page.getByTestId('selection-pill')).toContainText('1 selected');
    await expect
